...
|--------|-----------|-----------|------------|
| Physical Foundations | `references/physical-foundations.md` | 38 | Poiseuille, Laplace, diffusion, thermodynamics, saturation kinetics |
| Membranes & Transport | `references/membranes-transport.md` | 30 | Channels, pumps, osmosis, Donnan equilibrium |
| Excitable Cells | `references/excitable-cells.md` | 29 | Action potentials, Hodgkin-Huxley, muscle |
| Nervous System | `references/nervous-system.md` | 36 | Synapses, sensory, motor control |
| Cardiovascular | `references/cardiovascular.md` | 58 | Frank-Starling, hemodynamics, ECG |
| Respiratory | `references/respiratory.md` | 62 | Lung mechanics, V/Q matching, acid-base |
| Renal | `references/renal.md` | 42 | GFR, tubular function, countercurrent |
| Gastrointestinal | `references/gastrointestinal.md` | 41 | Secretion, absorption, motility |
| Endocrine | `references/endocrine.md` | 31 | Hormone kinetics, HPA axis, feedback |
...
registry itself: an OR-group is ≥2 equations declaring the **same `produces`
symbol** (a structural read, not a new value), and each alternative is tagged by
`kind` (definitional / approximation / measurement / model) so the typing never
overstates equivalence. It surfaces the consumers whose flat list conflates
alternatives as jointly-required — e.g. `systemic_oxygen_delivery` "needs" all
three cardiac-output methods when it needs CO by *any* one.

```python
from scripts import dependency_types as DT
print(DT.or_group("CO")["members"])          # {cardiac_output: definitional, ...fick/dilution: measurement}
print(DT.dependency_type("systemic_oxygen_delivery")["or"][0]["produces"])  # 'CO'
...
`scripts.eqforms` classifies every one of the 367 equations into one structural
FORM — `log_ratio`, `saturation`, `exponential`, `dynamics_ode`, `power_term`,
`algebraic` — grounded on two independent grounds (the execution-proven
`scripts.primitives` kernel *and* the written-formula marker), never hand-tagged.
It answers "what kind of equation is this, and which others share its shape".

```python
from scripts import eqforms as EF
print(EF.form_of("nernst_equation"))         # 'log_ratio'
print(EF.form_of("michaelis_menten"))        # 'saturation'
print(len(EF.by_form("exponential")))        # decay/charging forms
# doc-gate: expect 367
sum(len(EF.by_form(f)) for f in EF.FORMS)
```

### Recipe J — extensions: grounded relations beyond the Feher corpus

`scripts.extensions` is a tier SEPARATE from the canonical 367: definitional /
exact-math relations that go beyond Feher while holding the same discipline
(measurement-system response, thermoregulation heat balance, PK timing). No
device/drug/population number is committed — deferred quantities are listed as each
module's `NEEDS_SOURCE`; they are supplied at call time.

```python
from scripts.extensions import instrument as I, thermo as T
print(round(I.first_order_step_response.compute(t=1, tau=1), 4))   # 1-e^-1
# doc-gate: expect 0.6321 tol=0.001
round(I.first_order_step_response.compute(t=1, tau=1), 4)
```
