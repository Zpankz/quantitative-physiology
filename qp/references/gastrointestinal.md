...
Formula → qp:`peristalsis_velocity`
...
Formula → qp:`gastric_emptying_liquid`
Emptying half-time t₁/₂ = ln(2)/k ≈ 10–20 min (liquids).

For solids (lag phase + linear):
Formula → qp:`gastric_emptying_solid`
...
Formula → qp:`bile_acid_synthesis`
...
Formula → qp:`gastrin_acid_response`
...
Formula → qp:`cck_satiety`
CCK half-life ~2–3 min (rapid peptidase degradation).
...
Formula → qp:`extraction_ratio`

**Hepatic clearance**:
Formula → qp:`hepatic_clearance`
...
6 equation(s) added to this domain, each verified against Feher and a worked example.

### Energy Expenditure by Indirect Calorimetry (`energy_expenditure_indirect_calorimetry`)
- **Formula:** `M = 4.85 * VO2   (kcal/time; equivalently 20.3 kJ/L * VO2)`
- **LaTeX:** `M = 4.85\ \mathrm{kcal\,L^{-1}} \times V_{\mathrm{O_2}} = 20.3\ \mathrm{kJ\,L^{-1}} \times V_{\mathrm{O_2}}`
- **Import:** `from scripts.gastrointestinal.energy_balance import energy_expenditure_indirect_calorimetry` — or `from scripts.canonical_ids import load; load('energy_expenditure_indirect_calorimetry')`
- **Feher:** §8.6 — DETERMINATION OF ENERGY EXPENDITURE BY INDIRECT CALORIMETRY
- Indirect-calorimetry estimate of total metabolic rate (whole-body energy expenditure) from the rate of oxygen consumption, using the mixed-diet energy equivalent of oxygen (~4.85 kcal/L = 20.3 kJ/L).

### Carbohydrate Oxidation by Indirect Calorimetry (`carbohydrate_oxidation_indirect_calorimetry`)
- **Formula:** `c = -3.25*VO2 + 4.59*VCO2 - 3.75*n`
- **LaTeX:** `c = -3.25\,V_{\mathrm{O_2}} + 4.59\,V_{\mathrm{CO_2}} - 3.75\,n`
- **Import:** `from scripts.gastrointestinal.energy_balance import carbohydrate_oxidation_indirect_calorimetry` — or `from scripts.canonical_ids import load; load('carbohydrate_oxidation_indirect_calorimetry')`
- **Feher:** §8.6 — INDIRECT CALORIMETRY AND URINARY NITROGEN ALLOW ESTIMATION OF CATABOLISM OF MACRONUTRIENTS
- Estimates grams of carbohydrate oxidized from indirect calorimetry.

### Protein Catabolism from Urinary Nitrogen (`protein_oxidation_urinary_nitrogen`)
- **Formula:** `p = 6.25 * n`
- **LaTeX:** `p = 6.25\,n`
- **Import:** `from scripts.gastrointestinal.energy_balance import protein_oxidation_urinary_nitrogen` — or `from scripts.canonical_ids import load; load('protein_oxidation_urinary_nitrogen')`
- **Feher:** §8.6 — INDIRECT CALORIMETRY AND URINARY NITROGEN ALLOW ESTIMATION OF CATABOLISM OF MACRONUTRIENTS
- Estimates grams of protein catabolized (oxidized) from measured urinary urea nitrogen.

### Fat Oxidation Rate (Indirect Calorimetry) (`fat_oxidation_indirect_calorimetry`)
- **Formula:** `f = 1.69*VO2 - 1.69*VCO2 - 1.75*n`
- **LaTeX:** `f = 1.69\,V_{O_2} - 1.69\,V_{CO_2} - 1.75\,n`
- **Import:** `from scripts.gastrointestinal.energy_balance import fat_oxidation_indirect_calorimetry` — or `from scripts.canonical_ids import load; load('fat_oxidation_indirect_calorimetry')`
- **Feher:** §8.6 — INDIRECT CALORIMETRY AND URINARY NITROGEN ALLOW ESTIMATION OF CATABOLISM OF MACRONUTRIENTS
- Grams of fat oxidized, estimated by indirect calorimetry from O2 consumption, CO2 production, and urinary urea nitrogen (Feher Eq 8.6.8).

### Harris-Benedict BMR Equation (`harris_benedict_bmr`)
- **Formula:** `Men: BMR = 66.5 + 13.7*W + 5.0*H - 6.8*A ; Women: BMR = 655.1 + 9.56*W + 1.85*H - 4.7*A  [kcal/day; W kg, H cm, A yr]`
- **LaTeX:** `\text{Men: } \mathrm{BMR} = 66.5 + 13.7\,W + 5.0\,H - 6.8\,A \\ \text{Women: } \mathrm{BMR} = 655.1 + 9.56\,W + 1.85\,H - 4.7\,A`
- **Import:** `from scripts.gastrointestinal.energy_balance import harris_benedict_bmr` — or `from scripts.canonical_ids import load; load('harris_benedict_bmr')`
- **Feher:** §8.6 — EMPIRICAL FORMULAS FOR BMR
- Harris-Benedict (1919) empirical multiple-regression equation predicting basal metabolic rate (kcal/day) from weight, height, age, and sex.

### Mifflin-St. Jeor Resting Energy Expenditure (`mifflin_st_jeor_ree`)
- **Formula:** `REE_male = 5 + 9.99*W + 6.25*H - 4.92*A ; REE_female = -161 + 9.99*W + 6.25*H - 4.92*A  (kcal/day; W kg, H cm, A years)`
- **LaTeX:** `\text{Men: } REE = 5 + 9.99\,W + 6.25\,H - 4.92\,A;\quad \text{Women: } REE = -161 + 9.99\,W + 6.25\,H - 4.92\,A`
- **Import:** `from scripts.gastrointestinal.energy_balance import mifflin_st_jeor_ree` — or `from scripts.canonical_ids import load; load('mifflin_st_jeor_ree')`
- **Feher:** §8.6 — EMPIRICAL FORMULAS FOR BMR
- Mifflin-St.
...
- **stool_osmotic_gap** — `SOG = Osm_stool - 2(Na + K)` — Discriminates osmotic (>50-100) from secretory (<50) diarrhoea. Import: `from scripts.gastrointestinal.absorption import stool_osmotic_gap_equation` (or `load("stool_osmotic_gap")`).
